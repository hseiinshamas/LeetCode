class Solution {
    public String decodeMessage(String key, String message) {
        StringBuilder ans = new StringBuilder();//Using String Builder to append the string
            ...
            //After the first pass all the letters 
           //  of the key will be mapped with their 
           // respective original letters.
	       
        for (int i = 0; i < message.length(); i++) {
	            if (letters.containsKey(message.charAt(i))){
	                //Now replacing the letters of 
                    //the message with appropriate letter according to the key
	                ans.append(letters.get(message.charAt(i)));
	            }else{
	                ans.append(message.charAt(i));
	               
                    //This is for characters other 
                    //than the letters in the key example a space " "
	                //They will not be replaced by any 
                    //letters hence original letter is appended into the StringBuilder
	            }
	        }
	        return ans.toString();
    }
