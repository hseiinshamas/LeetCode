class Solution {
    ...
    public int[][] merge(int[][] intervals) {
        ...
        for (int[] curr : intervals) {
            ...
            if (mergedIntervals.isEmpty() || mergedIntervals.getLast()[1] < curr[0]) mergedIntervals.add(curr);
            // else if overlap exists then merge curren tinterval with the previous interval
            else mergedIntervals.getLast()[1] = Math.max(mergedIntervals.getLast()[1], curr[1]);
        }
